...
#include "utils.h"
#include <android/log.h> // 引入日志头文件

// 定义日志宏
#define LOG_TAG "PaddleRec"
    ...
    for (int n = 0; n < predict_shape[1]; n++) {
        ...
        if (argmax_idx > 0 && (!(n > 0 && argmax_idx == last_index))) {
            ...
            if (argmax_idx < charactor_dict.size()) {
                ...
                if (!logged) {
                    LOGE("严重错误: 识别结果索引越界! index=%d, dict_size=%d. 请检查字典文件是否与模型匹配。",
                         argmax_idx, (int) charactor_dict.size());
                    logged = true;
                }
                str_res += "?"; // 用问号代替未知字符
            }
